(ns cadence-eda.main
  "Kotodama WASM entrypoint for the Cadence_eda clean-room actor (L4) — Clojure port.
  ...
  facts (`cadence_eda.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cadence_eda")
(def tier "L4")
...
(def entity-specs
  [{:entity "Register"     :plural "registers"     :id-prefix "cadencee_reg"
    :fields [:name :widthBits :role]               :required [:name :widthBits]
    :coerce {:widthBits :int}          :refs {}}
   {:entity "Instruction"  :plural "instructions"  :id-prefix "cadencee_ins"
    :fields [:mnemonic :opcode :operands]          :required [:mnemonic :opcode]
    :coerce {}                         :refs {}}
   {:entity "Qubit"        :plural "qubits"        :id-prefix "cadencee_qub"
    :fields [:index :t1Us :t2Us :readoutError]     :required [:index :t1Us]
    :coerce {:index :int :t1Us :float :t2Us :float :readoutError :float} :refs {}}
   {:entity "Circuit"      :plural "circuits"      :id-prefix "cadencee_cir"
    :fields [:name :qubitCount :depth]             :required [:name :qubitCount]
    :coerce {:qubitCount :int :depth :int}         :refs {}}
   {:entity "Gate"         :plural "gates"         :id-prefix "cadencee_gat"
    :fields [:circuitId :name :targets]            :required [:name :targets]
    :coerce {}                         :refs {:circuitId "Circuit"}}
   {:entity "Job"          :plural "jobs"          :id-prefix "cadencee_job"
    :fields [:circuitId :shots :status]            :required [:shots :status]
    :coerce {:shots :int}              :refs {:circuitId "Circuit"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"cadence_eda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cadence_eda-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cadence_eda-compat/ready)
